...
&$PathToOpenSSLBinary req -x509 -nodes -new -sha256 -days 1024 -newkey rsa:2048 -keyout RootCA.key -out RootCA.pem -subj "/C=US/CN=Example-Root-CA"
&$PathToOpenSSLBinary x509 -outform pem -in RootCA.pem -out RootCA.crt

&$PathToOpenSSLBinary req -new -nodes -newkey rsa:2048 -keyout localhost.key -out localhost.csr -subj "/C=US/ST=YourState/L=YourCity/O=Example-Certificates/CN=localhost.local"
